//
//  TYSXStreamMediaMonitorViewController.m
//  tysx
//
//  Created by zwc on 14/11/19.
...
@interface TYSXStreamMediaMonitorViewController ()<HXCoreplotXAxisDataSource, HXCorePlotYAxisDataSource, UIScrollViewDelegate, HXBaseDataSource, UITableViewDataSource, UITableViewDelegate, OSSDrawCellDelegate>

@end

@implementation TYSXStreamMediaMonitorViewController {
    TYSXStreamMediaMonitorCtr *dataCtr;
...
- (id)initWithPlotName:(NSString *)_plotName {
    if (self = [super initWithPlotName:_plotName]) {
        dataCtr = (TYSXStreamMediaMonitorCtr *)cachePlotCtr;
    }
    return self;
}

+ (Class)cachePlotCtr {
    return [TYSXStreamMediaMonitorCtr class];
}
...
- (void)viewDidLoad {
    ...
    MESegmentedControl *segementedControl = [[MESegmentedControl alloc] initWithTitle:@[@"流量", @"次数", @"时长"]];
    segementedControl.x = 712;
   ...
   // segementedControl.selectedTitleColor = [UIColor colorWithHexString:@"f5f5f5"];
    ...
    sumPlotControl = [self linePlotControlWithTtile:@"图表：流量总数" lineColor:[UIColor redColor]];
    ...
    productPlotControl = [self linePlotControlWithTtile:@"图表：产品包流量统计" lineColor:[UIColor colorWithHexString:@"1a8291"]];
...
- (NSString *)unitConversionStringWithValue:(long long)value isShowPositiveSign:(BOOL)isShowPositiveSign {
    NSString *ret = nil;
    switch (dataCtr.currentType) {
        case 0:
            if (isShowPositiveSign) {
                ret = [NSString stringWithFormat:@"%+.2f", value * 1.0 / 1024 / 1024];
            }
            else {
                ret = [NSString stringWithFormat:@"%.2f", value * 1.0 / 1024 / 1024];
            }
        ...
        case 1:
            if (isShowPositiveSign) {
                ret = [NSString stringWithFormat:@"%+lld", value];
            }
            else {
                ret = [NSString stringWithFormat:@"%lld", value];
            }
...
- (void)drawCell:(OSSDrawCell *)cell rect:(CGRect)rect {
    ...
    [contentList addObject:[self unitConversionStringWithValue:[contentDic[kValueKey] longLongValue] isShowPositiveSign:NO]];
    ...
    [contentList addObject:[self unitConversionStringWithValue:[contentDic[kTongbiGapKey] longLongValue] isShowPositiveSign:YES]];
    [contentList addObject:[self unitConversionStringWithValue:[contentDic[kHuanbiGapKey] longLongValue] isShowPositiveSign:YES]];
...
- (void)changeSegementIndex:(MESegmentedControl *)segement {
    dataCtr.currentType = segement.selectedIndex;
    [self refreshAllView];
}
...
- (NSNumber *)plot:(HXBasePlot *)plot index:(NSInteger)index {
    if ([plot.plotName isEqualToString:@"图表：流量总数"]) {
        return dataCtr.sumTimesList[index];
    }
    else {
        return dataCtr.currentTimesList[index];
    }
}
...
- (void)contentView:(MYDrawContentView *)view drawRect:(CGRect)rect {
    ...
    [sumTypeName drawInRect:CGRectMake(0, offsetTop, rightEdge, 24) withAttributes:attr];
    ...
    [[NSString stringWithFormat:@"%@%@", [self unitConversionStringWithValue:sumCurrentValue isShowPositiveSign:NO], unitName] drawInRect:CGRectMake(0, offsetTop, rightEdge, 40) withAttributes:attr];
    ...
    [attr setValue:[UIColor grayColor] forKey:NSForegroundColorAttributeName];
    [[NSString stringWithFormat:@"%@%@", [self unitConversionStringWithValue:sumCurrentValue - sumYesterdayValue isShowPositiveSign:YES], unitName] drawInRect:CGRectMake(0, offsetTop, rightEdge, 20) withAttributes:attr];
    ...
    [attr setValue:[UIFont systemFontOfSize:18] forKey:NSFontAttributeName];
    [attr setValue:[UIColor grayColor] forKey:NSForegroundColorAttributeName];
    [[NSString stringWithFormat:@"%@%@", [self unitConversionStringWithValue:sumCurrentValue - sumLastWeakValue isShowPositiveSign:YES], unitName] drawInRect:CGRectMake(0, offsetTop, rightEdge, 20) withAttributes:attr];
    ...
    [attr setValue:[UIFont systemFontOfSize:24] forKey:NSFontAttributeName];
    [attr setValue:[UIColor darkGrayColor] forKey:NSForegroundColorAttributeName];
    [[NSString stringWithFormat:@"%@%@", [self unitConversionStringWithValue:selectedCurrentValue isShowPositiveSign:NO], unitName] drawInRect:CGRectMake(0, offsetTop, rightEdge, 40) withAttributes:attr];
    ...
    [attr setValue:[UIFont systemFontOfSize:18] forKey:NSFontAttributeName];
    [attr setValue:[UIColor grayColor] forKey:NSForegroundColorAttributeName];
    [[NSString stringWithFormat:@"%@%@", [self unitConversionStringWithValue:selectedCurrentValue - selectedYesterdayValue isShowPositiveSign:YES], unitName] drawInRect:CGRectMake(0, offsetTop, rightEdge, 20) withAttributes:attr];
    ...
    [attr setValue:[UIFont systemFontOfSize:18] forKey:NSFontAttributeName];
    [attr setValue:[UIColor grayColor] forKey:NSForegroundColorAttributeName];
    [[NSString stringWithFormat:@"%@%@", [self unitConversionStringWithValue:selectedCurrentValue - selectedLastWeakValue isShowPositiveSign:YES], unitName] drawInRect:CGRectMake(0, offsetTop, rightEdge, 20) withAttributes:attr];
    
    offsetTop = 520;
    NSArray *bottomTitles = nil;
    switch (dataCtr.currentType) {
        case 0:
            bottomTitles = @[@"产品", @"日期", @"流量(GB)", @"同比", @"环比", @"同比差(GB)", @"环比差(GB)", @"最后修改时间"];
            break;
        case 1:
            bottomTitles = @[@"产品", @"日期", @"次数(次)", @"同比", @"环比", @"同比差(次)", @"环比差(次)", @"最后修改时间"];
            break;
        case 2:
            bottomTitles = @[@"产品", @"日期", @"时长(秒)", @"同比", @"环比", @"同比差(秒)", @"环比差(秒)", @"最后修改时间"];
            break;
