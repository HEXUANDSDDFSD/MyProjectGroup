//
//  TYSXProductPackageOrderViewController.m
//  tysx
//
...
@interface TYSXProductPackageOrderViewController ()<HXCoreplotXAxisDataSource, HXCorePlotYAxisDataSource, UIScrollViewDelegate, HXBaseDataSource, UITableViewDataSource, UITableViewDelegate, OSSDrawCellDelegate>

@end

@implementation TYSXProductPackageOrderViewController {
    TYSXProductPackageOrderCtr *dataCtr;
...
- (id)initWithPlotName:(NSString *)_plotName {
    if (self = [super initWithPlotName:_plotName]) {
        dataCtr = (TYSXProductPackageOrderCtr *)cachePlotCtr;
    }
    return self;
}

+ (Class)cachePlotCtr {
    return [TYSXProductPackageOrderCtr class];
}
...
//    [self.view addSubview:segementedControl];
    ...
    sumPlotControl = [self linePlotControlWithTtile:@"图表：订购总数" lineColor:[UIColor redColor]];
    ...
    productPlotControl = [self linePlotControlWithTtile:@"图表：产品包订购统计" lineColor:[UIColor colorWithHexString:@"1a8291"]];
...
- (void)drawCell:(OSSDrawCell *)cell rect:(CGRect)rect {
    ...
    [contentList addObject:[NSString stringWithFormat:@"%lld", [contentDic[kOrderKey] longLongValue]]];
...
- (NSNumber *)plot:(HXBasePlot *)plot index:(NSInteger)index {
    if ([plot.plotName isEqualToString:@"图表：订购总数"]) {
...
- (void)contentView:(MYDrawContentView *)view drawRect:(CGRect)rect {
    ...
    [@"订购总数" drawInRect:CGRectMake(0, offsetTop, rightEdge, 24) withAttributes:attr];
